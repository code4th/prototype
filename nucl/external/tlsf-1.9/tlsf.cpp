...
//Allocate Information for Debug (MTRo)
namespace detail{

//	const static nc::uint32_t MagicCapValue = 0xffffffff;

	struct DebugMemHeader{
		nc::uint32_t	alignedGapForTrace_;	//blockのuser ptrの先頭になる、align設定によってallocCount_まで距離がある場合がある。
		nc::uint32_t	allocCount_;
		nc::uint32_t	alignedGap_;
	};

	struct DebugMem{
		DebugMemHeader	header_;
		nc::uint8_t		data_[1];
	}; 

	static nc::uint32_t	g_allocCount = 0;
}
...
static void default_walker(void* ptr, size_t size, int used, void* user)
{
	...
	{
#if 1
		detail::DebugMem* dbgPtr = (detail::DebugMem*)ptr;
		nc::uint32_t gap = dbgPtr->header_.alignedGapForTrace_;
		ptr = (void*)((nc::uint8_t*)ptr + gap);
		dbgPtr = (detail::DebugMem*)ptr;

		NC_TRACE("Addr=0x%p, Size=%d(%d), AllocCount=%d, Gap=%d, Dump={ 0x%08x, 0x%08x, 0x%08x, 0x%08x }"
			, ptr
			, size - sizeof(detail::DebugMemHeader) - gap
			, size
			, dbgPtr->header_.allocCount_
			, gap
			, *((nc::uint32_t*)dbgPtr->data_)
			, *(((nc::uint32_t*)dbgPtr->data_)+1)
			, *(((nc::uint32_t*)dbgPtr->data_)+2)
			, *(((nc::uint32_t*)dbgPtr->data_)+3) );
#else
		NC_TRACE("Addr=0x%p, Size=%d, Dump={ 0x%08x, 0x%08x, 0x%08x, 0x%08x }", ptr, size, *((nc::uint32_t*)ptr), *(((nc::uint32_t*)ptr)+1), *(((nc::uint32_t*)ptr)+2), *(((nc::uint32_t*)ptr)+3) );
#endif
	}
}
...
tlsf_pool tlsf_create(void* mem, size_t bytes)
{
	...
	{
		NC_TRACE("tlsf_create: Pool size must be between %d and %d bytes.\n", 
...
void* tlsf_malloc(tlsf_pool tlsf, size_t size)
{
	...
	size += sizeof( detail::DebugMemHeader );

	const size_t adjust = adjust_request_size(size, ALIGN_SIZE);

	block_header_t* block = block_locate_free(pool, adjust);
	detail::DebugMem* dbgPtr = (detail::DebugMem*)block_prepare_used(pool, block, adjust);

	dbgPtr->header_.alignedGapForTrace_ = dbgPtr->header_.alignedGap_ = 0;
	dbgPtr->header_.allocCount_ = detail::g_allocCount++;

	//NC_TRACE( "malloc : Type=%d, Cnt=%d, size=%d, addr=0x%08x", dbgPtr->header_.type_, dbgPtr->header_.allocCount_, dbgPtr->header_.size_, &dbgPtr->data_[0] );

	return &dbgPtr->data_[0];
...
void* tlsf_memalign(tlsf_pool tlsf, size_t align, size_t size)
{
	...
	size_t alignedDbgMemHeaderSize = align_up( sizeof(detail::DebugMemHeader), align );

	size += alignedDbgMemHeaderSize;
	const size_t adjust = adjust_request_size(size, ALIGN_SIZE);
#else
	const size_t adjust = adjust_request_size(size, ALIGN_SIZE);
#endif
	...
	*/
	...
	nc::uint8_t* rawPtr = (nc::uint8_t*)block_prepare_used(pool, block, adjust);

	size_t gapAlignedHeader = alignedDbgMemHeaderSize - sizeof( detail::DebugMemHeader );

	detail::DebugMem* dbgPtr = (detail::DebugMem*)(rawPtr + gapAlignedHeader);

	//確保したメモリの先頭4byteに入れる。
	*((nc::uint32_t*)rawPtr) = gapAlignedHeader;

	dbgPtr->header_.allocCount_ = detail::g_allocCount++;
	dbgPtr->header_.alignedGap_ = gapAlignedHeader;

	//NC_TRACE( "malloc : Type=%d, Cnt=%d, size=%d, addr=0x%08x", dbgPtr->header_.type_, dbgPtr->header_.g_allocCount, dbgPtr->header_.size_, &dbgPtr->data_[0] );
...
#if 1
		ptr = (void*)(((nc::uint8_t*)ptr) - sizeof( detail::DebugMemHeader ));

		detail::DebugMem* dbgPtr = (detail::DebugMem*)ptr;

		//NC_TRACE( "free : Type=%d, Cnt=%d, size=%d, addr=0x%08x", dbgPtr->header_.type_, dbgPtr->header_.g_allocCount, dbgPtr->header_.size_, &dbgPtr->data_[0] );

		ptr = ((nc::uint8_t*)ptr) - dbgPtr->header_.alignedGap_;
#endif
...
/* get memory amount of pool */
size_t tlsf_get_used_bytes( tlsf_pool tlsf )
{
	pool_t* pool = tlsf_cast(pool_t*, tlsf);
	return pool->used_bytes;
}
size_t tlsf_get_free_bytes( tlsf_pool tlsf )
{
	pool_t* pool = tlsf_cast(pool_t*, tlsf);
	return pool->free_bytes;
}

size_t tlsf_get_total_bytes( tlsf_pool tlsf )
{
	pool_t* pool = tlsf_cast(pool_t*, tlsf);
